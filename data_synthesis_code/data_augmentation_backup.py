...
def get_random_position(center, radius, s1, s2, template_size, image_size):
    """
    ...
    if not (0 <= s1 < s2):
        raise ValueError("s1和s2必须满足: 0 <= s1 < s2")
    ...
    if max_distance + min_required_space > min(image_h//2, image_w//2):
        raise ValueError("给定的半径范围过大，无法放置模板")
    
    max_attempts = 1000  # 设置最大尝试次数
    attempts = 0
    # 使用系统时间作为随机数种子,每次运行程序时生成不同的随机序列
    random.seed(None)
    ...
    return (-1, -1)  # 返回无效坐标,调用方需要判断坐标的有效性
...
def process_templates(template_dir, template_label_dir, target_image_dir, target_label_dir,
                     sample_ratio=0.1, radius_range=(0.5, 0.8), max_attempts=100, 
    ...
    Args:
        ...
        radius_range: 圆环区域的半径范围比例，(内圈比例, 外圈比例)
        max_attempts: 最大尝试次数
    ...
    print(f"模板的数量: {len(template_files)}")
    print(f"选择的目标文件数量: {len(selected_targets)}")
    print(f"选择比例: {sample_ratio:.2%}")
    ...
    for template_file in tqdm(template_files, desc="处理模板"):
        ...
        # 对每个模板进行旋转和翻转预处理
        for transform_mode in range(6):  # 0-原图, 1-水平翻转, 2-上下翻转, 3-90度, 4-180度, 5-270度
            # 获取变换后的模板和标签
                ...
                for _ in range(max_attempts):
                    ...
                    for _, existing_box in existing_boxes:
                        if check_box_overlap(target_box, existing_box, target_img.shape[:2]):
                            ...
                            has_overlap = True
                            break
                        if check_box_distance(target_box, existing_box, target_img.shape[:2], min_center_dist=10):
                            has_overlap = True
                            break
                    ...
                    result = blend_template(
                        ...
                        pos=pos,
                        blur_size=np.random.randint(30, 35),
                        opacity=np.random.uniform(0.8, 0.95),
                        dilate_size=blend_dilate_size,
                        save_process=False,
                        debug_dir=debug_dir
                    )
                    ...
                    if debug:
                        ...
                        for i, line in enumerate(lines):
                            ...
                            # 最后一行（新合成的标签）使用红色，其他使用绿色
                            color = (0, 0, 255) if i == len(lines)-1 else (0, 255, 0)
                            visualize_rotated_box(debug_img, points, color=color)
...
def main():
    ...
    target_image_dir = "/mnt/nfs/AOI_detection/ccm/data/v6/after_screening/liuqin_data/train_data_sampling_yolo/images/train"
    target_label_dir = "/mnt/nfs/AOI_detection/ccm/data/v6/after_screening/liuqin_data/train_data_sampling_yolo/labels/train"
    debug_dir = "/mnt/nfs/AOI_detection/ccm/data/v6/after_screening/liuqin_data/debug_output"
    # 参数设置
    debug = True
    sample_ratio = 0.1
    radius_range = (1.1, 1.25)
    max_attempts = 100
    ...
    process_templates(
        ...
        debug=debug,
        debug_dir=debug_dir,
        debug_template_index=debug_template_index  # 传入新参数
    )
