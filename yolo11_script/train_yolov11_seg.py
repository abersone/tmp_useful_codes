...
def train_yolo11_seg(config):
    # 创建带时间戳的实验输出目录
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    exp_name = f"seg_exp_{config['model_size']}_{config['img_size']}_{timestamp}"
    ...
    if config.get('model'):
        ...
        print(f"training use the yolo official model: {config['model_path']}")
    ...
    train_args = {
        ...
        # 内存优化参数
        "amp": config.get('amp', True),               # 自动混合精度训练（节省内存）
        "workers": config.get('workers', 4),          # 减少数据加载器工作进程数
        "cache": config.get('cache', False),          # 不缓存图像到内存
    ...
    print(f"分割模型训练完成。结果保存在: {output_dir}")
