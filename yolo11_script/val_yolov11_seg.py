...
def validate_yolo11_seg(config):
    """验证YOLO11分割模型"""
    # 创建带时间戳的验证输出目录
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    val_name = f"val_seg_{config['model_size']}_{config['img_size']}_{timestamp}"
    ...
    if config.get('model'):
        ...
        model = YOLO(config['model_path'])  # 使用默认的分割模型
    ...
    try:
        ...
        print(f"  results.seg: {hasattr(results, 'seg')}")
        ...
        if hasattr(results, 'seg'):
            print(f"  seg.map50: {hasattr(results.seg, 'map50')}")
            print(f"  seg.mp: {hasattr(results.seg, 'mp')}")
            print(f"  seg.mr: {hasattr(results.seg, 'mr')}")
    ...
    results_summary = {
        ...
        "metrics": {
            ...
            "seg_mAP50": float(results.seg.map50) if hasattr(results.seg, 'map50') else None,
            "seg_mAP50-95": float(results.seg.map) if hasattr(results.seg, 'map') else None,
            "seg_precision": float(results.seg.mp) if hasattr(results.seg, 'mp') else None,
            "seg_recall": float(results.seg.mr) if hasattr(results.seg, 'mr') else None,
        }
    }
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        ...
        f.write("\n分割性能指标:\n")
        f.write(f"  分割mAP@0.5: {results_summary['metrics']['seg_mAP50']:.4f}\n")
        f.write(f"  分割mAP@0.5:0.95: {results_summary['metrics']['seg_mAP50-95']:.4f}\n")
        f.write(f"  分割精确率: {results_summary['metrics']['seg_precision']:.4f}\n")
        f.write(f"  分割召回率: {results_summary['metrics']['seg_recall']:.4f}\n")

    print(f"分割模型验证完成。结果保存在: {output_dir}")
    ...
    print(f"\n分割性能指标:")
    print(f"  分割mAP@0.5: {results_summary['metrics']['seg_mAP50']:.4f}")
    print(f"  分割mAP@0.5:0.95: {results_summary['metrics']['seg_mAP50-95']:.4f}")
    print(f"  分割精确率: {results_summary['metrics']['seg_precision']:.4f}")
    print(f"  分割召回率: {results_summary['metrics']['seg_recall']:.4f}")
