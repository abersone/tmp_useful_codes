...
def process_templates(template_dir, template_label_dir, target_image_dir, target_label_dir,
    ...
    for template_file in tqdm(template_files, desc="处理模板"):
        ...
        # 2.5 对每个模板进行旋转和翻转预处理
        for transform_mode in range(3):  # 0-原图, 1-水平翻转, 2-上下翻转, 3-90度, 4-180度, 5-270度
            # 2.5.1 获取变换后的模板和标签
                ...
                for _ in range(max_attempts):
                    ...
                    for _, existing_box in existing_boxes:
                        if check_box_overlap(target_box, existing_box, target_img.shape[:2]):
                            ...
                            has_overlap = True
                            break
                        if check_box_distance(target_box, existing_box, target_img.shape[:2], min_center_dist=100):
                            has_overlap = True
                            break
                    ...
                    result = blend_template(
                        ...
                        dilate_size=2 * np.random.randint(1, blend_dilate_size // 2) + 1,
                        save_process=False,
                        debug_dir=debug_dir
                    )
