...
def filter_annotations_by_difference(image_path, shapes, kernel_size=5, contrast_threshold=20, merge_distance=40, min_area=100):
    ...
    for shape in shapes:
        ...
        # cv2.imwrite(f"debug_diff_{shapes.index(shape)}.jpg", debug_mask)
    ...
    for shape in filtered_shapes:
        ...
        if shape["shape_type"] == "circle":
            ...
            if len(points) == 2:
                ...
                print(f"警告：在文件 {json_file} 中，矩形标签 {label} 的点数不正确，跳过此标注。")
                continue
        else:            
            # 计算最小面积矩形
...
def process_dataset(labelme_path, yolo_path, class_names, test_mode=False, threshold=20, kernel_size=5, merge_distance=40, min_area=100):
    ...
    print(f"共处理 {len(json_files)} 个文件")
...
def split_dataset(yolo_path, train_ratio=1):
    images_path = Path(yolo_path) / "images"
    labels_path = Path(yolo_path) / "labels"
...
def generate_dataset_yaml(yolo_obb_path, class_names):
    creation_time = datetime.now().isoformat()  # 获取当前时间
    yaml_content = {
...
def main(test_mode=False):
    labelme_path = r"C:\Users\Eugene\Desktop\golden_wire_dataset\BaiKang_YiBiaoZhu\baikang_seg_ball_labelme"
    yolo_obb_path = r"C:\Users\Eugene\Desktop\golden_wire_dataset\BaiKang_YiBiaoZhu\baikang_seg_ball_yolo"
    test_output_path = yolo_obb_path + r"\vis"
    class_names = ["ball", "pad"]
    # class_names = ["dacuoxian", "feixian", "qiupian", "shuangxian", "wuqiu", "xianhu", "wuxian"]
